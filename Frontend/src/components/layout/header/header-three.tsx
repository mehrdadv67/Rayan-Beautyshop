...
export default function Header() {
  ...
  const { data: liveMenu } = useMenuQuery();
  const desktopMenu = liveMenu?.menu ?? site_header.menu;
  ...
  return (
    ...
    >
      <div className='fixed z-20 w-full h-16 px-4 text-gray-700 transition duration-200 ease-in-out bg-white innerSticky body-font sm:h-20 lg:h-36 xl:h-40 ltr:pl-4 rtl:pr-4 ltr:md:pl-0 rtl:md:pr-0 ltr:lg:pl-6 rtl:lg:pr-6 ltr:pr-4 ltr:lg:pr-6 rtl:pl-4 rtl:lg:pl-6 md:px-8 2xl:px-16'>
        <div className='flex items-center justify-between mx-auto max-w-[1920px] h-full lg:h-20 xl:h-24 w-full relative before:absolute before:w-screen before:h-px before:bg-[#F1F1F1] before:bottom-0'>
          <button
            aria-label='Menu'
          ...
          <div className='flex flex-shrink-0 transition-all duration-200 ease-in-out transform ltr:mr-3 rtl:ml-3 languageSwitcher lg:hidden'>
            <LanguageSwitcher />
          </div>
